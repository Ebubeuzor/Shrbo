    ...
    <div className="transition-5 flex flex-col gap-5 md:gap-0 cursor-default bg-slate-800 z-[1000] bottom-0  overflow-hidden md:rounded-xl fixed h-full w-full md:w-[320px] md:h-[450px] lg:h-[420px] md:bottom-[103px] md:right-6 md:max-h-[calc(100% - 48px)] md:max-w-[calc(100% - 48px)] " style={{
       ...
       <div className=" close-button text-white text-end z-50 w-full top-2 right-2 absolute flex justify-end items-center cursor-pointer pr-1  " onClick={close}>
         ...
         <div className=' relative md:top-[15%] top-[12%] flex font-medium flex-col gap-4  md:gap-2 pt-6 mt-12 md:mt-2 md:p-4  p-6 text-slate-50  h-full bg-slate-50 overflow-hidden ' >
                ...
                <button className=' rounded-xl bg-orange-400 p-3 md:p-2 hover:bg-orange-300 transition-colors hover:text-slate-600   '  onClick={() => handleOptionClick('Customer Service')}>Customer Service</button>
                <button className=' rounded-xl bg-orange-400 p-3 md:p-2 hover:bg-orange-300 transition-colors hover:text-slate-600   '  onClick={() => handleOptionClick('Technical Support')}>Technical Support</button>
                <button className=' rounded-xl bg-orange-400 p-3 md:p-2 hover:bg-orange-300 transition-colors hover:text-slate-600   '  onClick={() => handleOptionClick('Booking Issue')}>Booking Issuee</button>
                <button className=' rounded-xl bg-orange-400 p-3 md:p-2 hover:bg-orange-300 transition-colors hover:text-slate-600   '  onClick={() => handleOptionClick('Billing')}>Billing</button>
        </div>
    </div>
  );
};
